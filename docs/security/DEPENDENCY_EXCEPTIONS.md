...
| GHSA-w5hq-g745-h8pq | `uuid` through the Xcode project-generation utility                 | Build-time only; CharTalk does not call vulnerable buffer-based UUID variants; native projects are generated from reviewed config                            | Expo config plugins move to `uuid>=11.1.1`                                        |
