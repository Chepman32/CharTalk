...
export default function StoriesScreen() {
  ...
  const charactersById = useMemo(
    () =>
      new Map(
        discoveryCatalog.characters.map(character => [
  ...
  const renderStory = useCallback(
    ({ item: story }: ListRenderItemInfo<Story>) => (
      <View style={columnCount === 2 ? styles.gridCell : undefined}>
        {story.storyId === recommendedStoryId ? (
          <View style={styles.recommended}>
            <Sparkle color={nativeColors.inverse} size={14} weight="fill" />
        ...
        <StoryCard
          story={story}
          character={charactersById.get(story.characterId)}
    ...
    [
      charactersById,
      columnCount,
      nativeColors.inverse,
      openStory,
      recommendedStoryId,
      styles,
    ],
  )
  const renderStorySeparator = useCallback(
    () => <View style={styles.storySeparator} />,
    [styles.storySeparator],
  )

  return (
    <Screen
      contentStyle={styles.virtualizedScreen}
      scroll={false}
      testID="stories-screen"
    >
      <FlatList
        key={`catalog-${columnCount}`}
        data={stories}
        renderItem={renderStory}
        keyExtractor={story => story.storyId}
        numColumns={columnCount}
        columnWrapperStyle={columnCount === 2 ? styles.gridRow : undefined}
        ...
        ItemSeparatorComponent={renderStorySeparator}
        ListHeaderComponent={
          <View style={styles.listHeader}>
                  ...
                  if (!runContent) {
                    ...
                    return (
                      <View key={run.runId} style={styles.continueCardMissing}>
                        ...
                        <Button
                          label={
                            bundledRun ? 'Продолжить' : 'Восстановить версию'
                          }
                          variant="quiet"
                          onPress={() =>
                            bundledRun
            ...
            {branchedStoryIds.length > 0 ? (
              <View style={styles.section}>
                ...
                <Text color={nativeColors.textSecondary}>
                  Отдельная карта ваших решений — без спойлеров ненайденных
                  путей.
                </Text>
...
const createStyles = (nativeColors: ThemeColorAliases) =>
  StyleSheet.create({
    ...
    gridRow: { gap: spacing[5] },
    gridCell: { flex: 1 },
    storySeparator: { height: spacing[5] },
    recommended: {
      position: 'absolute',
      zIndex: 2,
      right: spacing[4],
