...
interface ContinuationFrame {
  ...
  reactions: readonly [string, string, string, string]
}
...
const continuationIntents = [
  'verify-continuation',
  'set-continuation-boundary',
  'support-continuation',
  'hold-continuation-question',
] as const

const continuationFrames: readonly ContinuationFrame[] = [
  {
    prompt: 'Появилась новая деталь. Её нужно сравнить с исходными записями.',
    choices: [
      'Давай сверим её с исходником.',
      'Сначала решим, чего мы пока не обсуждаем.',
      'Хочешь, проверим записи вдвоём?',
      'Пока ничего не решаем. Сначала сверим записи.',
    ],
    reactions: [
      'Сверяю. В исходнике время указано иначе.',
      'Хорошо. Эту часть пока не трогаем; смотрим только на запись.',
      'Давай. Я открою исходник, а ты следи за датами.',
      'Согласна. Сначала сравним записи, выводы потом.',
    ],
  },
  {
    prompt: 'В записях смешались факты и догадки. Их пора разделить.',
    choices: [
      'Выпишем отдельно то, что знаем точно.',
      'Я не хочу обсуждать догадку как доказанный факт.',
      'Давай по шагам: сначала только факты.',
      'С выводами подождём. Что здесь подтверждено?',
    ],
    reactions: [
      'Записываю отдельно: что видели, что слышали и что только предполагаем.',
      'Согласна. Догадку помечаю вопросом, а не фактом.',
      'Тогда по очереди. Я читаю записи, ты останавливаешь на спорных местах.',
      'Так честнее. Пока у нас два факта и ни одного готового ответа.',
    ],
  },
  {
    prompt:
      'Одного совпадения мало. Нужен источник, который не видел наших записей.',
    choices: [
      'Поищем независимое подтверждение.',
      'Без имён и лишних подробностей. Спросим только о времени.',
      'Давай разделимся: ты проверишь время, я — место.',
      'Не будем принимать совпадение за доказательство. Ищем ещё одно.',
    ],
    reactions: [
      'Проверю второй источник. Он пока ничего не знает о нашей версии.',
      'Договорились. В вопросе останутся только дата и время.',
      'Хорошо. Я беру место, тебе отправляю отметки времени.',
      'Согласна. Одного совпадения мало; ищем ещё одно.',
    ],
  },
  {
    prompt:
      'Продолжать наугад не стоит. Нужен простой шаг, который можно отменить.',
    choices: [
      'Начнём с действия, которое легко проверить и отменить.',
      'Рисковать ради одной версии не стоит.',
      'Скажи, чем помочь прямо сейчас.',
      'Ничего необратимого. Сначала самый простой шаг.',
    ],
    reactions: [
      'Тогда делаем копию и работаем только с ней.',
      'Согласна. Никаких ночных поездок и звонков наугад.',
      'Возьми список. Я пока сохраню исходники.',
      'Хорошо. Сначала копия, потом проверка; оригиналы не трогаем.',
    ],
  },
  {
    prompt:
      'Последовательность не сходится. Похоже, из записей выпал один кусок.',
    choices: [
      'Пройдёмся по времени ещё раз и найдём пропуск.',
      'Не станем заполнять пробел догадкой.',
      'Читай по порядку, а я отмечу разрывы.',
      'Если куска нет, так и запишем. Без красивой версии.',
    ],
    reactions: [
      'Нашла разрыв: между двумя отметками нет шести минут.',
      'Верно. Оставляю пустое место и пометку «нет данных».',
      'Начинаю с первой строки. Останови меня, где время перескочит.',
      'Так и пишу: шесть минут не восстановлены.',
    ],
  },
  {
    prompt:
      'До следующего разговора лучше сразу договориться, кто что проверяет.',
    choices: [
      'Я запишу, что ещё нужно проверить.',
      'И договоримся, к чему пока не возвращаемся.',
      'Присылай всё как есть. Разберём по порядку.',
      'Если новых фактов не будет, гадать не станем.',
    ],
    reactions: [
      'Запиши три пункта: время, источник и последнюю правку.',
      'Договорились. До завтра не звоним и никого не обвиняем.',
      'Отправляю папку без сортировки. Начнём с дат.',
      'Согласна. Нет новых фактов — нет нового вывода.',
    ],
  },
] as const
...
function buildStoryNodes(blueprint: StoryBlueprint): ContentNode[] {
  ...
  for (let stage = 3; stage <= SAMPLE_STORY_CHOICE_POINTS; stage += 1) {
    for (let branchIndex = 0; branchIndex < 4; branchIndex += 1) {
      ...
      const choiceCandidates = continuationIntents.map(
        (intent, choiceIndex) => {
          ...
          return choice(
            ...
            frame.choices[choiceIndex]!,
            intent,
            nextNodeId,
            `${branch.openingIntent}:${stage}:${intent}`,
          )
        },
      ) as [ChoiceCandidate, ChoiceCandidate, ChoiceCandidate, ChoiceCandidate]
      ...
      continuationIntents.forEach((_intent, choiceIndex) => {
        nodes.push({
  ...
  branches: [
    {
      ...
      followUps: [
        ...
        {
          text: 'Сохрани копию и уходи. Завтра решишь, что делать дальше.',
          intent: 'set-boundary',
          reaction:
            'Она закрывает таблицу, но пересылает архив на защищённый диск.',
          endingTitle: 'Дверь закрыта вовремя',
          epilogue:
            'Ира ушла из редакции с копией документов и вернулась к разговору утром.',
        },
        {
    ...
    },
    {
      ...
      followUps: [
        ...
        {
          text: 'Сначала поешь. На пустой желудок и думается хуже.',
          intent: 'ground',
          reaction:
            'Ира достаёт из шкафа сухое печенье и смеётся ровно один раз.',
          endingTitle: 'Пауза у чайника',
          epilogue: 'После чая и печенья Ира снова открыла документы.',
        },
      ],
    },
  ],
  ...
  branches: [
    {
      ...
      followUps: [
        ...
        },
        {
          ...
          epilogue:
            'Журнал проходов и парковка указали на одни и те же семь минут.',
        },
        {
      ...
      followUps: [
        {
          ...
          epilogue:
            'Скриншот оказался подделкой: его сделали не в интерфейсе системы.',
        },
        {
        ...
        },
        {
          ...
          epilogue:
            'Версию охранника подтвердили записью в журнале и показаниями ночной смены.',
        },
      ],
    },
  ],
      ...
      followUps: [
        {
          ...
          epilogue:
            'Гул объяснил работу лифта, но происхождение трёх ударов осталось неясным.',
        },
        {
          ...
          epilogue: 'Связь с остановкой лифта сузила поиск до шестого этажа.',
        },
        {
      ...
      followUps: [
        {
          ...
          epilogue:
            'Датчик двери оказался неисправен; обращение зарегистрировали.',
        },
        {
          ...
          epilogue: 'Утром Дина проверила лифт вместе с техником и соседом.',
        },
        {
          ...
          epilogue:
            'Курьер объяснил повторяющиеся вызовы; его маршрут подтвердили камеры.',
        },
        {
      ...
      followUps: [
        {
          ...
          epilogue:
            'Два микрофона показали: источник был во дворе, а стена только отражала звук.',
        },
        {
        ...
        },
        {
          ...
          epilogue: 'Рекордер сработал по таймеру, пока Дина ждала у соседки.',
        },
        {
          ...
          epilogue:
            'Соседи независимо друг от друга описали один и тот же металлический хлопок.',
        },
      ],
    },
    {
      ...
      prompt:
        'Можно уехать сейчас или дождаться соседку в освещённом холле. Я точно не останусь в квартире одна.',
      followUps: [
        {
          ...
          epilogue:
            'Дина уехала, а на следующий день продолжила проверку при свете.',
        },
        {
        ...
        },
        {
          ...
          endingTitle: 'Проверка днём',
          epilogue: 'Дина ушла на ночь, а днём вернулась вместе с техником.',
        },
      ],
    },
  ],
...
export const sampleContentPackage: ContentPackage = {
  ...
  characters: [
    ...
    },
    {
      ...
      description:
        'Приземлённая мистика о звуке в ночном доме и поиске, который лучше продолжать днём.',
  ...
  stories: [
    ...
    },
    {
      ...
      title: 'Семь минут',
      premise:
        'Камера выключилась всего на семь минут — и журналы проходов перестали сходиться.',
    ...
    },
    {
      ...
      title: 'Три удара',
      premise:
        'В пустой квартире слышны три удара. Дина ищет источник звука и не собирается рисковать ради записи.',
