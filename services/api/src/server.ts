...
const adminToken = process.env.RAZVILKA_ADMIN_TOKEN
if (production && (!adminToken || adminToken.length < 24)) {
  throw new Error(
    'RAZVILKA_ADMIN_TOKEN must contain at least 24 characters in production',
  )
}

const cmsRoleTokens = (['writer', 'editor', 'qa', 'publisher'] as const).reduce(
  (tokens, role) => {
    const value = process.env[`RAZVILKA_CMS_${role.toUpperCase()}_TOKEN`]
...
for (const [role, token] of Object.entries(cmsRoleTokens)) {
  if (production && token.length < 24) {
    throw new Error(
      `RAZVILKA_CMS_${role.toUpperCase()}_TOKEN must contain at least 24 characters in production`,
    )
  }
  if (token === adminToken) {
    throw new Error(
      `RAZVILKA_CMS_${role.toUpperCase()}_TOKEN must not duplicate RAZVILKA_ADMIN_TOKEN`,
    )
  }
}

const publishEnabled = process.env.RAZVILKA_PUBLISH_ENABLED === 'true'
const privateKeyPath = process.env.RAZVILKA_SIGNING_PRIVATE_KEY_FILE
const signingKeyId = process.env.RAZVILKA_SIGNING_KEY_ID
if (production && publishEnabled && !signingKeyId) {
  throw new Error(
    'RAZVILKA_SIGNING_KEY_ID is required when production publishing is enabled',
  )
}
const privateKey = publishEnabled
  ? privateKeyPath
    ...
    : production
      ? (() => {
          throw new Error(
            'RAZVILKA_SIGNING_PRIVATE_KEY_FILE is required when production publishing is enabled',
          )
        })()
      : generateKeyPairSync('ed25519').privateKey
  : undefined

const contentPath = process.env.RAZVILKA_CONTENT_PACKAGE_PATH
const publicKeyPath = process.env.RAZVILKA_SIGNING_PUBLIC_KEY_FILE
const bundledContent = contentPath
  ? (() => {
      if (!publicKeyPath) {
        throw new Error(
          'RAZVILKA_SIGNING_PUBLIC_KEY_FILE is required with a content package',
        )
      }
  ...
  : production
    ? (() => {
        throw new Error(
          'RAZVILKA_CONTENT_PACKAGE_PATH is required in production',
        )
      })()
    : sampleContentPackage

const contentAssetRootValue = process.env.RAZVILKA_CONTENT_ASSET_ROOT
if (production && !contentAssetRootValue) {
  throw new Error('RAZVILKA_CONTENT_ASSET_ROOT is required in production')
}
...
const databasePath = resolve(
  process.env.RAZVILKA_API_DB_PATH ?? './var/razvilka-api.db',
)
mkdirSync(dirname(databasePath), { recursive: true, mode: 0o700 })
const store = new SqliteApiStore(databasePath, bundledContent)
const syncEnabled = process.env.RAZVILKA_SYNC_ENABLED === 'true'
const syncToken = process.env.RAZVILKA_SYNC_TOKEN
const syncAccountId = process.env.RAZVILKA_SYNC_ACCOUNT_ID ?? 'account.default'
if (syncEnabled && !/^[a-zA-Z0-9][a-zA-Z0-9._:-]{0,199}$/.test(syncAccountId)) {
  throw new Error(
    'RAZVILKA_SYNC_ACCOUNT_ID must be a stable account identifier',
  )
}
if (production && syncEnabled && (!syncToken || syncToken.length < 24)) {
  throw new Error(
    'RAZVILKA_SYNC_TOKEN must contain at least 24 characters when sync is enabled in production',
  )
}
...
const allowedOrigins = (
  process.env.RAZVILKA_ALLOWED_ORIGINS ?? 'http://localhost:5173'
)
  .split(',')
...
const hostname = process.env.RAZVILKA_API_HOST ?? '127.0.0.1'
const server = serve({ fetch: app.fetch, hostname, port })

console.info(
  JSON.stringify({
    level: 'info',
    message: 'Развилка API listening',
    hostname,
    port,
  }),
)
